...
app.post("/books", (req, res) => {
  ...
  if (new_book.title && new_book.author && new_book.year) {
    ...
    res.status(400).json({ success: false, msj: "Fallo al crear libro!", new_book})
  }
});
...
app.put("/books", (req, res) => {
  ...
  if(index != -1){ // Existe el libro ?
    ...
    res.status(400).json({ success: false, msj: "Error. Libro con encontrado!. Campos que puedes utilizar: (title, new_title, author, year). Es obligatorio usar title", book_data})
  }
